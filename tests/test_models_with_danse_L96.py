...
from parameters_opt import get_parameters, f_lorenz, f_lorenz_ukf, f_chen, f_chen_ukf, f_rossler, f_rossler_ukf, \
    J_test, delta_t_L96, decimation_factor_L96, get_H_DANSE
from bin.generate_data import Lorenz96SSM
...
def test_on_ssm_model(device='cpu', learnable_model_files=None, test_data_file=None, 
    ...
    if not os.path.isfile(test_data_file):
        ...
        if "Lorenz96SSM" in ssm_type:
            delta = delta_t_L96 # If decimate is True, then set this delta to 1e-5 and run it for long time
            delta_d = delta_t_L96 / decimation_factor_L96
            ssm_model_test = Lorenz96SSM(
                n_states=m, n_obs=n, delta=delta, delta_d=delta_d, F_mu=ssm_model_test_dict['F_mu'], decimate=decimate, 
                mu_w=np.zeros((n,)), method=ssm_model_test_dict['method'], H=get_H_DANSE(type_=ssm_type_test, n_states=m, n_obs=n)
            )
            decimation_factor = decimation_factor_L96
    ...
    print("sigma_e2: {}dB, smnr: {}dB, delta_t: {}, n: {}".format(sigma_e2_dB_test, smnr_dB_test, delta_t_L96, n), file=orig_stdout)

    print("Fed to Model-based filters: ")
    print("sigma_e2: {}dB, smnr: {}dB, delta_t: {}, n: {}".format(sigma_e2_dB_test, smnr_dB_test, delta_t_L96, n))
    ...
    if "danse_semisupervised" in models_list:
        ...
        print("Model file {}".format(model_file_saved_danse_semisupervised), file=orig_stdout)
    ...
    torch.save([X, Y, X_estimated_filtered, X_estimated_filtered_danse_semisupervised], os.path.join(os.path.join(figs_dir, dirname, evaluation_mode), 'test_results_models_smnr_{}_n{}.pt'.format(smnr_dB_test, n)))
    #torch.save([X, Y, Pk_estimated_filtered, Pk_estimated_filtered_danse_semisupervised], os.path.join(os.path.join(figs_dir, dirname, evaluation_mode), 'test_results_models_cov_smnr_{}_n{}.pt'.format(smnr_dB_test, n)))  
    ...
    plot_3d_state_trajectory(
        ...
        m='b-', 
        savefig_name="./{}/{}/{}/{}_x_true_sigmae2_{}dB_smnr_{}dB_n{}.pdf".format(
            figs_dir, dirname, evaluation_mode, ssm_type_test, sigma_e2_dB_test, smnr_dB_test, n), savefig=True)
     
    plot_3d_measurement_trajectory(
        Y=torch.squeeze(Y[id_, :, :], 0).numpy(), 
        legend='$\\mathbf{y}^{true}$', 
        m='r-', 
        savefig_name="./{}/{}/{}/{}_y_true_sigmae2_{}dB_smnr_{}dB_n{}.pdf".format(
            figs_dir, dirname, evaluation_mode, ssm_type_test, sigma_e2_dB_test, smnr_dB_test, n), savefig=True)
    ...
    if "danse" in models_list:
        plot_3d_state_trajectory(
            ...
            m='k-', 
            savefig_name="./{}/{}/{}/{}_x_danse_sigmae2_{}dB_smnr_{}dB_n{}.pdf".format(
                figs_dir, dirname, evaluation_mode, ssm_type_test, sigma_e2_dB_test, smnr_dB_test, n), savefig=True)
    ...
    if "danse_semisupervised" in models_list:
        plot_3d_state_trajectory(
            ...
            m='k-', 
            savefig_name="./{}/{}/{}/{}_x_semidanse_sigmae2_{}dB_smnr_{}dB_n{}.pdf".format(
                figs_dir, dirname, evaluation_mode, ssm_type_test, sigma_e2_dB_test, smnr_dB_test, n), savefig=True)
    
    if "danse_supervised" in models_list:
        plot_3d_state_trajectory(
            X=torch.squeeze(X_estimated_filtered_danse_supervised[id_], 0).numpy(), 
            legend='$\\hat{\mathbf{x}}_{DANSE-Sup.}$', 
            m='k-', 
            savefig_name="./{}/{}/{}/{}_x_danse_supervised_sigmae2_{}dB_smnr_{}dB_n{}.pdf".format(
                figs_dir, dirname, evaluation_mode, ssm_type_test, sigma_e2_dB_test, smnr_dB_test, n), savefig=True)
    ...
    if "dmm_st-l" in models_list:
        plot_3d_state_trajectory(
            ...
            m='k-', 
            savefig_name="./{}/{}/{}/{}_x_dmm_causal_sigmae2_{}dB_smnr_{}dB_n{}.pdf".format(
                figs_dir, dirname, evaluation_mode, ssm_type_test, sigma_e2_dB_test, smnr_dB_test, n), savefig=True)
    ...
    plot_state_trajectory_axes_all(
        ...
        savefig=True,
        savefig_name="./{}/{}/{}/AxesAll_sigmae2_{}dB_smnr_{}dB_n{}.pdf".format(
            figs_dir, dirname, evaluation_mode, sigma_e2_dB_test, smnr_dB_test, n)
    )
    '''
    plot_state_trajectory_axes_all(
        X=torch.squeeze(X[id_,:,:],0).numpy(), 
        ...
        savefig=True,
        savefig_name="./{}/{}/{}/AxesAll_errors_sigmae2_{}dB_smnr_{}dB_n{}.pdf".format(
            figs_dir, dirname, evaluation_mode, sigma_e2_dB_test, smnr_dB_test, n)
    )
    '''
    plot_state_trajectory_axes_all_L96(
        X=torch.squeeze(X[id_,:,:],0).numpy(), 
        X_est_DANSE=torch.squeeze(X_estimated_filtered[id_],0).numpy() if "danse" in models_list else None,
        X_est_DANSE_Supervised=torch.squeeze(X_estimated_filtered_danse_supervised[id_],0).numpy() if "danse_supervised" in models_list else None,
        X_est_SemiDANSE=torch.squeeze(X_estimated_filtered_danse_semisupervised[id_],0).numpy() if "danse_semisupervised" in models_list else None,
        #X_est_KNET=torch.squeeze(X_estimated_filtered_knet[id_], 0).numpy() if "kalmannet" in models_list else None,
        savefig=True,
        savefig_name="./{}/{}/{}/AxesAll_L96_sigmae2_{}dB_smnr_{}dB_n{}.pdf".format(
            figs_dir, dirname, evaluation_mode, sigma_e2_dB_test, smnr_dB_test, n)
    )
    ...
    plot_state_trajectory_w_lims(
        ...
        X_est_DMM=torch.squeeze(X_estimated_filtered_dmm_causal[id_],0).numpy() if "dmm_st-l" in models_list else None,
        X_est_DMM_std=np.sqrt(torch.diagonal(torch.squeeze(Pk_estimated_filtered_dmm_causal[id_,:,:,:], 0), offset=0, dim1=1,dim2=2).numpy()) if "dmm_st-l" in models_list else None,
        X_est_SemiDANSE=torch.squeeze(X_estimated_filtered_danse_semisupervised[id_],0).numpy() if "danse_semisupervised" in models_list else None,
        ...
        savefig=True,
        savefig_name="./{}/{}/{}/Axes_w_lims_sigmae2_{}dB_smnr_{}dB_n{}.pdf".format(
            figs_dir, dirname, evaluation_mode, sigma_e2_dB_test, smnr_dB_test, n)
    )
    '''
...
if __name__ == "__main__":
    ...
    for nsup in nsup_arr:
        ...
        if mode == 'low' or mode == 'high':
            ...
            bias = None
            p = None
            evaluation_mode = 'ModTest_diff_n_nsup_{}_smnr_{}dB_Ntrain_{}_Ttrain_{}_Ntest_{}_Ttest_{}_refactored'.format(nsup, smnr_dB_test, N_train, T_train, N_test, T_test)

        figs_dir = "figs_new_2026-4-23_danse_semidanse" #"figs_new_{}_danse_semidanse".format(str(datetime.date.today()))
        dirname = "{}SSMn{}".format(ssm_name,m)
        os.makedirs('./{}/{}/{}'.format(figs_dir, dirname, evaluation_mode), exist_ok=True)

        n_arr = [15, 10, 5, 2]
        n_dict_arr = ["{}".format(n) for n in n_arr]

        list_of_models_comparison = ["ls", "danse", "danse_semisupervised"]
        ...
        metrics_multidim_mat = np.zeros((len(list_of_metrics), len(list_of_models_comparison), len(n_arr)))

        model_file_saved_dict = {
                                "danse":dict.fromkeys(n_dict_arr, {}), 
                                #"kalmannet":dict.fromkeys(n_dict_arr, {}),
                                #"dmm_st-l":dict.fromkeys(n_dict_arr, {}),
                                #"danse_supervised":dict.fromkeys(n_dict_arr, {}), 
                                "danse_semisupervised":dict.fromkeys(n_dict_arr, {}), 
                                }
        
        test_data_file_dict = {}

        for j, n_label in enumerate(n_dict_arr):
            ssmtype_j = "{}SSMn{}".format(ssm_name, n_arr[j]) # Hardcoded for {}SSMrn{} (random H low rank), {}SSMn{} (deterministic H low rank), 
            test_data_file_dict[n_label] = "./data/synthetic_data/test_trajectories_m_{}_n_{}_{}_data_T_{}_N_{}_sigmae2_{}dB_smnr_{}dB.pkl".format(
                m, n_arr[j], ssmtype_j, T_test, N_test, sigma_e2_dB_test, smnr_dB_test
                )
        ...
        for key in model_file_saved_dict.keys():
            for j, n_label in enumerate(n_dict_arr):
                ...
                if saved_model == "danse_semisupervised" and not nsup is None:
                    model_file_saved_dict[key][n_label] = glob.glob("./models/*{}_{}_*nsup_{}_m_{}_n_{}_*T_{}_N_{}*sigmae2_{}dB_smnr_{}dB*/*best*".format(
                        ssmtype_j, saved_model, nsup, m, n_arr[j], T_train, N_train_actual, sigma_e2_dB_test, smnr_dB_test))[-1]
                elif saved_model == "KNetUoffline":
                    model_file_saved_dict[key][n_label] = glob.glob("./models/*{}_{}_*m_{}_*n_{}_*T_{}_N_{}*sigmae2_{}dB_smnr_{}dB*/*best*".format(
                        ssmtype_j, saved_model, m, n_arr[j], T_train, N_train_actual, sigma_e2_dB_test, smnr_dB_test))[-1]
                else:
                    model_file_saved_dict[key][n_label] = glob.glob("./models/*{}_{}_*opt_gru_m_{}_*n_{}_*T_{}_N_{}*sigmae2_{}dB_smnr_{}dB*/*best*".format(
                        ssmtype_j, saved_model, m, n_arr[j], T_train, N_train_actual, sigma_e2_dB_test, smnr_dB_test))[-1]

        test_logfile = "./{}/{}/{}/{}_test_{}_nsup_{}_T_{}_N_{}_ModTest.log".format(figs_dir, dirname, evaluation_mode, ssmtype_j, evaluation_mode, nsup, T_test, N_test)
        test_jsonfile = "./{}/{}/{}/{}_test_{}_nsup_{}_T_{}_N_{}_ModTest.json".format(figs_dir, dirname, evaluation_mode, ssmtype_j, evaluation_mode, nsup, T_test, N_test)

        for idx_n, n_label in enumerate(n_dict_arr):
            
            test_data_file_i = test_data_file_dict[n_label]
            learnable_model_files_i = {key: model_file_saved_dict[key][n_label] for key in model_file_saved_dict.keys()}
            print(test_data_file_i)
            print(learnable_model_files_i)
            ...
            metrics_dict_i = test_on_ssm_model(device=device, learnable_model_files = learnable_model_files_i, test_data_file=test_data_file_i, 
                ...
                dirname=dirname, figs_dir=figs_dir)
            ...
            for idx_metric, idx_model_name in itertools.product(list(range(len(list_of_metrics))), list(range(len(list_of_models_comparison)))):
                metrics_multidim_mat[idx_metric, idx_model_name, idx_n] = metrics_dict_i[list_of_metrics[idx_metric]][list_of_models_comparison[idx_model_name]]
        ...
        for idx_display_metric in display_metrics:
            ...
            if list_of_metrics[idx_display_metric] != "time_elapsed":
                ...
                for j, model_name in enumerate(list_of_models_comparison):
                    plt.errorbar(n_arr, metrics_multidim_mat[idx_display_metric, j, :], fmt=list_of_display_fmts[j], yerr=metrics_multidim_mat[idx_display_metric+1, j, :],  linewidth=1.5, label="{}".format(model_name.upper()))
                plt.xlabel('n')
                ...
                plt.tight_layout()
                #plt.subplot(212)
                tikzplotlib.save('./{}/{}/{}/{}_vs_n_{}.tex'.format(figs_dir, dirname, evaluation_mode, list_of_metrics[idx_display_metric].upper(), ssm_name))
                plt.savefig('./{}/{}/{}/{}_vs_n_{}.pdf'.format(figs_dir, dirname, evaluation_mode, list_of_metrics[idx_display_metric].upper(), ssm_name))
            else:
                plt.figure()
                for j, model_name in enumerate(list_of_models_comparison):
                    plt.plot(n_arr, metrics_multidim_mat[idx_display_metric, j, :], list_of_display_fmts[j], linewidth=1.5, label="{}".format(model_name.upper()))
                plt.xlabel('n')
