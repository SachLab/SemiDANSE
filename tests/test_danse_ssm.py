...
from src.danse_unkcw import DANSE_UNKCW
...
def test_danse_ssm(
    ...
    ssm_dict, est_dict = get_parameters(
        n_states=ssm_model_test.n_states, n_obs=ssm_model_test.n_obs, device=device
    )
...
def test_danse_unkcw_ssm(
    Y_test,
    ssm_model_test,
    model_file_saved_danse_unkcw,
    Cw_test=None,
    rnn_type="gru",
    device="cpu",
):
    ...
    print("DANSE (Unknown-CW) Model file: {}".format(model_file_saved_danse_unkcw))

    start_time_danse = timer()
    danse_unkcw_model.load_state_dict(
        torch.load(model_file_saved_danse_unkcw, map_location=device)
    )
    danse_unkcw_model = push_model(nets=danse_unkcw_model, device=device)
    danse_unkcw_model.eval()
    ...
    with torch.no_grad():
        ...
        (
            X_estimated_pred_danse_unkcw,
            Pk_estimated_pred_danse_unkcw,
            X_estimated_filtered_danse_unkcw,
            Pk_estimated_filtered_danse_unkcw,
            Cw_estimated_filtered_danse_unkcw
        ) = danse_unkcw_model.compute_predictions(Y_test_batch)

    time_elapsed_danse_unkcw = timer() - start_time_danse
    ...
    return (
        ...
        X_estimated_filtered_danse_unkcw,
        Pk_estimated_filtered_danse_unkcw,
        time_elapsed_danse_unkcw,
    )


def test_danse_supervised_ssm(
    Y_test,
    ssm_model_test,
    ...
    danse_supervised_model.load_state_dict(
        torch.load(model_file_saved_danse_supervised, map_location=device)
    )
    ...
    with torch.no_grad():
        ...
        (
            X_estimated_pred_supervised,
            Pk_estimated_pred_supervised,
            X_estimated_filtered_supervised,
            Pk_estimated_filtered_supervised,
        ) = danse_supervised_model.compute_predictions(Y_test_batch, Cw_test_batch)
...
def test_danse_semisupervised_ssm(
    Y_test,
    ssm_model_test,
    ...
    danse_semisupervised_model.load_state_dict(
        torch.load(model_file_saved_danse_semisupervised, map_location=device)
    )
    danse_semisupervised_model = push_model(
        nets=danse_semisupervised_model, device=device
    )
    ...
    with torch.no_grad():
        ...
        (
            X_estimated_pred_semisupervised,
            Pk_estimated_pred_semisupervised,
            X_estimated_filtered_semisupervised,
            Pk_estimated_filtered_semisupervised,
        ) = danse_semisupervised_model.compute_predictions(Y_test_batch, Cw_test_batch)
    ...
    return (
        ...
        X_estimated_filtered_semisupervised,
        Pk_estimated_filtered_semisupervised,
        time_elapsed_danse_semisupervised,
    )
