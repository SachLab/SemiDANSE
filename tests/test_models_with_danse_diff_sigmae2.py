...
def test_on_ssm_model(device='cpu', learnable_model_files=None, test_data_file=None, 
            ...
            dirname=None, kappa=None, figs_dir=None):
    ...
    ssm_type, rnn_type, _, m, n, T, _, sigma_e2_dB, smnr_dB = parse("{}_danse_semisupervised_opt_{}_nsup_{}_m_{:d}_n_{:d}_T_{:d}_N_{:d}_sigmae2_{:f}dB_SMNR_{:f}dB", model_file_saved_danse_semisupervised.split('/')[-2])
    ...
    print("sigma_e2: {}dB, smnr: {}dB, kappa: {}".format(sigma_e2_dB_test, smnr_dB_test, kappa))
    print("sigma_e2: {}dB, smnr: {}dB, kappa: {}".format(sigma_e2_dB_test, smnr_dB_test, kappa), file=orig_stdout)

    ###########################################################################################################
    # NOTE: The following three lines of code are only for rapid testing for debugging of code,
    ...
    torch.save([X, Y, X_estimated_filtered_danse_supervised, X_estimated_filtered_danse_semisupervised], os.path.join(os.path.join(figs_dir, dirname, evaluation_mode), 'test_results_models_kappa_{}_smnr_{}.pt'.format(kappa, smnr_dB_test)))
    torch.save([X, Y, Pk_estimated_filtered_danse_supervised, Pk_estimated_filtered_danse_semisupervised], os.path.join(os.path.join(figs_dir, dirname, evaluation_mode), 'test_results_models_cov_kappa_{}_smnr_{}.pt'.format(kappa, smnr_dB_test)))  
    ...
    #####################################################################################################################################################################
    # Plot the result
    id_ = np.random.choice(X.shape[0],replace=False)
    ...
    plot_3d_state_trajectory(
        ...
        savefig_name="./{}/{}/{}/{}_x_true_sigmae2_{}dB_smnr_{}dB.pdf".format(
            figs_dir, dirname, evaluation_mode, ssm_type_test, sigma_e2_dB_test, smnr_dB_test), savefig=True)
    ...
    if "danse" in models_list:
        plot_3d_state_trajectory(
            X=torch.squeeze(X_estimated_filtered[id_], 0).numpy(), 
            legend='$\\hat{\mathbf{x}}_{DANSE}$', 
            m='k-', 
            savefig_name="./{}/{}/{}/{}_x_danse_sigmae2_{}dB_smnr_{}dB.pdf".format(
                figs_dir, dirname, evaluation_mode, ssm_type_test, sigma_e2_dB_test, smnr_dB_test), savefig=True)
    ...
    if "danse_semisupervised" in models_list:
        plot_3d_state_trajectory(
            X=torch.squeeze(X_estimated_filtered_danse_semisupervised[id_], 0).numpy(), 
            legend='$\\hat{\mathbf{x}}_{SemiDANSE}$', 
            m='k-', 
            savefig_name="./{}/{}/{}/{}_x_semidanse_sigmae2_{}dB_smnr_{}dB.pdf".format(
                figs_dir, dirname, evaluation_mode, ssm_type_test, sigma_e2_dB_test, smnr_dB_test), savefig=True)
    ...
    plot_state_trajectory_axes_all(
        X=torch.squeeze(X[id_,:,:],0).numpy(), 
        X_est_EKF=torch.squeeze(X_estimated_ekf[id_,:,:],0).numpy() if "ekf" in models_list else None, 
        X_est_UKF=torch.squeeze(X_estimated_ukf[id_,:,:],0).numpy() if "ukf" in models_list else None, 
        #X_est_DANSE=torch.squeeze(X_estimated_filtered[id_],0).numpy() if "danse" in models_list else None,
        X_est_SemiDANSE=torch.squeeze(X_estimated_filtered_danse_semisupervised[id_],0).numpy() if "danse_semisupervised" in models_list else None,
        X_est_DMM=torch.squeeze(X_estimated_filtered_dmm_causal[id_],0).numpy() if "dmm_st-l" in models_list else None,
        X_est_KNET=torch.squeeze(X_estimated_filtered_knet[id_], 0).numpy() if "kalmannet" in models_list else None,
        savefig=True,
        savefig_name="./{}/{}/{}/AxesAll_sigmae2_{}dB_smnr_{}dB.pdf".format(
            figs_dir, dirname, evaluation_mode, sigma_e2_dB_test, smnr_dB_test)
    )

    plot_state_trajectory_axes_all(
        X=torch.squeeze(X[id_,:,:],0).numpy(), 
        X_est_EKF=torch.squeeze(X[id_,:,:] - X_estimated_ekf[id_,:,:],0).numpy() if "ekf" in models_list else None, 
        X_est_UKF=torch.squeeze(X[id_,:,:] - X_estimated_ukf[id_,:,:],0).numpy() if "ukf" in models_list else None, 
        #X_est_DANSE=torch.squeeze(X[id_,:,:] - X_estimated_filtered[id_],0).numpy() if "danse" in models_list else None,
        X_est_SemiDANSE=torch.squeeze(X[id_,:,:] - X_estimated_filtered_danse_semisupervised[id_],0).numpy() if "danse_semisupervised" in models_list else None,
        X_est_DMM=torch.squeeze(X[id_,:,:] - X_estimated_filtered_dmm_causal[id_],0).numpy() if "dmm_st-l" in models_list else None,
        X_est_KNET=torch.squeeze(X[id_,:,:] - X_estimated_filtered_knet[id_], 0).numpy() if "kalmannet" in models_list else None,
        savefig=True,
        savefig_name="./{}/{}/{}/AxesAll_errors_sigmae2_{}dB_smnr_{}dB.pdf".format(
            figs_dir, dirname, evaluation_mode, sigma_e2_dB_test, smnr_dB_test)
    )
    ...
    plot_state_trajectory_w_lims(
        X=torch.squeeze(X[id_,:,:],0).numpy(), 
        X_est_UKF=torch.squeeze(X_estimated_ukf[id_,:,:],0).numpy() if "ukf" in models_list else None, 
        X_est_UKF_std=np.sqrt(torch.diagonal(torch.squeeze(Pk_estimated_ukf[id_,:,:,:], 0), offset=0, dim1=1,dim2=2).numpy()) if "ukf" in models_list else None, 
        X_est_DANSE=torch.squeeze(X_estimated_filtered[id_],0).numpy() if "danse" in models_list else None,
        X_est_DANSE_std=np.sqrt(torch.diagonal(torch.squeeze(Pk_estimated_filtered[id_,:,:,:], 0), offset=0, dim1=1,dim2=2).numpy()) if "danse" in models_list else None,
        X_est_SemiDANSE=torch.squeeze(X_estimated_filtered_danse_semisupervised[id_],0).numpy() if "danse_semisupervised" in models_list else None,
        X_est_SemiDANSE_std=np.sqrt(torch.diagonal(torch.squeeze(Pk_estimated_filtered_danse_semisupervised[id_,:,:,:], 0), offset=0, dim1=1,dim2=2).numpy()) if "danse_semisupervised" in models_list else None,
        savefig=True,
    ...
    plot_meas_trajectory_w_lims(
        ...
        savefig=True,
        savefig_name="./{}/{}/{}/Meas_y_lims_sigmae2_{}dB_smnr_{}dB.pdf".format(
            figs_dir, dirname, evaluation_mode, sigma_e2_dB_test, smnr_dB_test)
    )
    '''
    #plt.show()
...
if __name__ == "__main__":
    ...
    T_test = 2000
    N_test = 100
    sigma_e2_dB_nominal_arr = np.array([-20.0, -10.0, -5.0, 0.0, 5.0])
    device = 'cpu'
    smnr_dB_test = 10.0
    nsup = 20
    kappa = nsup / N_train

    for sigma_e2_dB_nominal in sigma_e2_dB_nominal_arr:
        ...
        if mode == 'low' or mode == 'high':
            ...
            bias = None
            p = None
            evaluation_mode = 'full_opt_diff_sigma_e2_dB_kappa_{:.2f}_smnr_{}dB_Ntrain_{}_Ttrain_{}_sigma_e2_dB_nominal_{}dB'.format(nsup/N_train, smnr_dB_test, N_train, T_train, sigma_e2_dB_nominal)

        ssmtype = "{}SSMrn{}".format(ssm_name, n) if n < m else "{}SSM".format(ssm_name) # Hardcoded for {}SSMrn{} (random H low rank), {}SSMn{} (deterministic H low rank), 
        dirname = "{}SSMrn{}x{}".format(ssm_name,m,n)
        #figs_dir = "figs_new_{}_diff_sigma_e2_dB".format(str(datetime.date.today()))
        figs_dir = "figs_new_2024-12-02_diff_sigma_e2_dB_matched"
        os.makedirs('./{}/{}/{}'.format(figs_dir, dirname, evaluation_mode), exist_ok=True)

        sigma_e2_dB_arr = np.array([-20.0, -10.0, -5.0, 0.0, 5.0])
        sigma_e2_dB_dict_arr = ["{}dB".format(sigma_e2_dB) for sigma_e2_dB in sigma_e2_dB_arr]

        list_of_models_comparison = ["ls", "danse_semisupervised"]
        list_of_display_fmts = ["gp-.", "bo--", "co-"]
        ...
        #        metrics_dict[metric][model_name] = np.zeros((len(kappa_arr)))
        
        metrics_multidim_mat = np.zeros((len(list_of_metrics), len(list_of_models_comparison), len(sigma_e2_dB_arr)))

        model_file_saved_dict = {
                                #"danse":dict.fromkeys(kappa_dict_arr, {}), 
                                #"kalmannet":dict.fromkeys(kappa_dict_arr, {}),
                                #"dmm_st-l":dict.fromkeys(kappa_dict_arr, {}),
                                #"danse_supervised":dict.fromkeys(sigma_e2_dB_dict_arr, {}), 
                                "danse_semisupervised":dict.fromkeys(sigma_e2_dB_dict_arr, {}), 
                                }
        
        test_data_file_dict = {}

        for j, sigma_e2_dB_label in enumerate(sigma_e2_dB_dict_arr):
            test_data_file_dict[sigma_e2_dB_label] = "./data/synthetic_data/test_trajectories_m_{}_n_{}_{}_data_T_{}_N_{}_sigmae2_{}dB_smnr_{}dB.pkl".format(
                m, n, ssmtype, T_test, N_test, sigma_e2_dB_arr[j], smnr_dB_test
                )
        ...
        for key in model_file_saved_dict.keys():
            for j, sigma_e2_dB_label in enumerate(sigma_e2_dB_dict_arr):
                ...
                if "matched" in figs_dir:
                    if saved_model == "danse_semisupervised" and not nsup is None:
                        model_file_saved_dict[key][sigma_e2_dB_label] = glob.glob("./models/*{}_{}_*nsup_{}_m_{}_n_{}_*T_{}_N_{}*sigmae2_{}dB_smnr_{}dB*/*best*".format(
                            ssmtype, saved_model, nsup, m, n, T_train, N_train_actual, sigma_e2_dB_arr[j], smnr_dB_test))[-1]
                    elif saved_model == "KNetUoffline":
                        model_file_saved_dict[key][sigma_e2_dB_label] = glob.glob("./models/*{}_{}_*m_{}_*n_{}_*T_{}_N_{}*sigmae2_{}dB_smnr_{}dB*/*best*".format(
                            ssmtype, saved_model, m, n, T_train, N_train_actual, sigma_e2_dB_arr[j], smnr_dB_test))[-1]
                    else:
                        model_file_saved_dict[key][sigma_e2_dB_label] = glob.glob("./models/*{}_{}_*opt_gru_m_{}_*n_{}_*T_{}_N_{}*sigmae2_{}dB_smnr_{}dB*/*best*".format(
                            ssmtype, saved_model, m, n, T_train, N_train_actual, sigma_e2_dB_arr[j], smnr_dB_test))[-1]
                else:
                    if saved_model == "danse_semisupervised" and not nsup is None:
                        model_file_saved_dict[key][sigma_e2_dB_label] = glob.glob("./models/*{}_{}_*nsup_{}_m_{}_n_{}_*T_{}_N_{}*sigmae2_{}dB_smnr_{}dB*/*best*".format(
                            ssmtype, saved_model, nsup, m, n, T_train, N_train_actual, sigma_e2_dB_nominal, smnr_dB_test))[-1]
                    elif saved_model == "KNetUoffline":
                        model_file_saved_dict[key][sigma_e2_dB_label] = glob.glob("./models/*{}_{}_*m_{}_*n_{}_*T_{}_N_{}*sigmae2_{}dB_smnr_{}dB*/*best*".format(
                            ssmtype, saved_model, m, n, T_train, N_train_actual, sigma_e2_dB_nominal, smnr_dB_test))[-1]
                    else:
                        model_file_saved_dict[key][sigma_e2_dB_label] = glob.glob("./models/*{}_{}_*opt_gru_m_{}_*n_{}_*T_{}_N_{}*sigmae2_{}dB_smnr_{}dB*/*best*".format(
                            ssmtype, saved_model, m, n, T_train, N_train_actual, sigma_e2_dB_nominal, smnr_dB_test))[-1]

        test_logfile = "./{}/{}/{}/{}_test_{}_nsup_{}_T_{}_N_{}_diff_sigmae2.log".format(figs_dir, dirname, evaluation_mode, ssmtype, evaluation_mode, nsup, T_test, N_test)
        test_jsonfile = "./{}/{}/{}/{}_test_{}_nsup_{}_T_{}_N_{}_diff_sigmae2.json".format(figs_dir, dirname, evaluation_mode, ssmtype, evaluation_mode, nsup, T_test, N_test)

        print(model_file_saved_dict)

        for idx_kappa, sigma_e2_dB_label in enumerate(sigma_e2_dB_dict_arr):
            
            test_data_file_i = test_data_file_dict[sigma_e2_dB_label]
            learnable_model_files_i = {key: model_file_saved_dict[key][sigma_e2_dB_label] for key in model_file_saved_dict.keys()}
            ...
            metrics_dict_i = test_on_ssm_model(device=device, learnable_model_files = learnable_model_files_i, test_data_file=test_data_file_i, 
                ...
                dirname=dirname, kappa=kappa, figs_dir=figs_dir)
            ...
            for idx_metric, idx_model_name in itertools.product(list(range(len(list_of_metrics))), list(range(len(list_of_models_comparison)))):
                metrics_multidim_mat[idx_metric, idx_model_name, idx_kappa] = metrics_dict_i[list_of_metrics[idx_metric]][list_of_models_comparison[idx_model_name]]
        ...
        for idx_display_metric in display_metrics:

            if list_of_metrics[idx_display_metric] != "time_elapsed":
                plt.figure()
                for j, model_name in enumerate(list_of_models_comparison):
                    plt.errorbar(sigma_e2_dB_arr, metrics_multidim_mat[idx_display_metric, j, :], fmt=list_of_display_fmts[j], yerr=metrics_multidim_mat[idx_display_metric+1, j, :],  linewidth=1.5, label="{}".format(model_name.upper()))
                plt.xlabel('$\\sigma_{e}^{2}$')
                plt.ylabel('{} (in dB)'.format(list_of_metrics[idx_display_metric].upper()))
                plt.grid(True)
                plt.legend()
                plt.tight_layout()
                #plt.subplot(212)
                tikzplotlib.save('./{}/{}/{}/{}_vs_sigmae2_{}.tex'.format(figs_dir, dirname, evaluation_mode, list_of_metrics[idx_display_metric].upper(), ssm_name))
                plt.savefig('./{}/{}/{}/{}_vs_sigmae2_{}.pdf'.format(figs_dir, dirname, evaluation_mode, list_of_metrics[idx_display_metric].upper(), ssm_name))
            else:
                plt.figure()
                for j, model_name in enumerate(list_of_models_comparison):
                    plt.plot(sigma_e2_dB_arr, metrics_multidim_mat[idx_display_metric, j, :], list_of_display_fmts[j], linewidth=1.5, label="{}".format(model_name.upper()))
                plt.xlabel('$\\sigma_{e}^{2}$')
