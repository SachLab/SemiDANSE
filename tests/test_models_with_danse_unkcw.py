...
from test_danse_ssm import test_danse_ssm, test_danse_semisupervised_ssm, test_danse_supervised_ssm, test_danse_unkcw_ssm
...
def test_on_ssm_model(device='cpu', learnable_model_files=None, test_data_file=None, 
    ...
    model_file_saved_danse_unkcw = learnable_model_files["danse_unkcw"] if "danse_unkcw" in models_list else None
    ...
    #####################################################################################################################################################################
     #####################################################################################################################################################################
    # Estimator: DANSE - Unknown Covariance (Cw)
    if "danse_unkcw" in models_list:
        print("Testing DANSE (Unknown Cov.) ...", file=orig_stdout)
        X_estimated_pred_danse_unkcw, Pk_estimated_pred_danse_unkcw, X_estimated_filtered_danse_unkcw, Pk_estimated_filtered_danse_unkcw, time_elapsed_danse_unkcw = test_danse_unkcw_ssm(
            Y_test=Y, ssm_model_test=ssm_model_test, model_file_saved_danse_unkcw=model_file_saved_danse_unkcw, 
            Cw_test=Cw, rnn_type=rnn_type, device=device
        )
        X_estimated_dict["danse_unkcw"]["est"] = X_estimated_filtered_danse_unkcw
        X_estimated_dict["danse_unkcw"]["est_cov"] = Pk_estimated_filtered_danse_unkcw
        Y_estimated_pred_danse_unkcw, Py_estimated_pred_danse_unkcw = get_y_pred_linear_meas(X_estimated_pred_test=X_estimated_pred_danse_unkcw,
                                                                            Pk_estimated_pred_test=Pk_estimated_pred_danse_unkcw,
                                                                            Cw_test=Cw, 
                                                                            ssm_model_test=ssm_model_test
                                                                        )
    
    else:
        X_estimated_pred_danse_unkcw, Pk_estimated_pred_danse_unkcw, X_estimated_filtered_danse_unkcw, Pk_estimated_filtered_danse_unkcw, time_elapsed_danse_unkcw = None, None, None, None, None
        Y_estimated_pred_danse_unkcw, Py_estimated_pred_danse_unkcw = None, None
    ...
    torch.save([X, Y, X_estimated_ekf, X_estimated_ukf, X_estimated_filtered, X_estimated_filtered_danse_semisupervised], os.path.join(os.path.join(figs_dir, dirname, evaluation_mode), 'test_results_models_smnr_{}.pt'.format(smnr_dB_test)))
    torch.save([X, Y, Pk_estimated_ekf, Pk_estimated_ukf, Pk_estimated_filtered, Pk_estimated_filtered_danse_semisupervised], os.path.join(os.path.join(figs_dir, dirname, evaluation_mode), 'test_results_models_cov_smnr_{}.pt'.format(smnr_dB_test)))  
    ...
    for metric_name in metrics_list:
        ...
        if not metric_fn is None:
            metrics_dict_for_one_smnr[metric_name] = {
                ...
                "danse_unkcw": metric_fn(X[:,:,:], X_estimated_filtered_danse_unkcw[:,0:,:]).numpy().item() if "danse_unkcw" in models_list else None,
    ...
    metrics_dict_for_one_smnr["time_elapsed"] = {
                ...
                "danse_unkcw": time_elapsed_danse_unkcw if "danse_unkcw" in models_list else None,
    ...
    if "danse_unkcw" in models_list:
        plot_3d_state_trajectory(
            X=torch.squeeze(X_estimated_filtered_danse_unkcw[id_], 0).numpy(), 
            legend='$\\hat{\mathbf{x}}_{DANSE-UnknownCw}$', 
            m='k-', 
            savefig_name="./{}/{}/{}/{}_x_danse_unkcw_sigmae2_{}dB_smnr_{}dB.pdf".format(
                figs_dir, dirname, evaluation_mode, ssm_type_test, sigma_e2_dB_test, smnr_dB_test), savefig=True)
...
if __name__ == "__main__":
    ...
    for nsup in nsup_arr:
        ...
        figs_dir = "figs_new_2025-04-25_danse_unkcw" #"figs_new_{}_danse_semidanse".format(str(datetime.date.today()))
        ...
        list_of_models_comparison = ["ls", "danse", "danse_unkcw"]
        ...
        model_file_saved_dict = {
                                ...
                                "danse_unkcw":dict.fromkeys(smnr_dB_dict_arr, {}),
        ...
        for key in model_file_saved_dict.keys():
            for j, smnr_dB_label in enumerate(smnr_dB_dict_arr):
                ...
                if saved_model == "danse_semisupervised" and not nsup is None:
                ...
                elif saved_model == "danse_unk" and not nsup is None:
                    model_file_saved_dict[key][smnr_dB_label] = glob.glob("./models/*{}_{}_*m_{}_n_{}_*T_{}_N_{}*sigmae2_{}dB_smnr_{}dB*/*unk*best*".format(
                        ssmtype, saved_model, nsup, m, n, T_train, N_train_actual, sigma_e2_dB_test, smnr_dB_arr[j]))[-1]
                    ...
                    model_file_saved_dict[key][smnr_dB_label] = glob.glob("./models/*{}_{}_opt_gru_m_{}_*n_{}_*T_{}_N_{}*sigmae2_{}dB_smnr_{}dB*/*best*".format(
                        ssmtype, saved_model, m, n, T_train, N_train_actual, sigma_e2_dB_test, smnr_dB_arr[j]))[-1]

        test_logfile = "./{}/{}/{}/{}_test_{}_nsup_{}_T_{}_N_{}_ModTest.log".format(figs_dir, dirname, evaluation_mode, ssmtype, evaluation_mode, nsup, T_test, N_test)
        test_jsonfile = "./{}/{}/{}/{}_test_{}_nsup_{}_T_{}_N_{}_ModTest.json".format(figs_dir, dirname, evaluation_mode, ssmtype, evaluation_mode, nsup, T_test, N_test)
        ...
        for idx_display_metric in display_metrics:
            ...
            if list_of_metrics[idx_display_metric] != "time_elapsed":
                ...
                plt.tight_layout()
                #plt.subplot(212)
                tikzplotlib.save('./{}/{}/{}/{}_vs_SMNR_{}.tex'.format(figs_dir, dirname, evaluation_mode, list_of_metrics[idx_display_metric].upper(), ssm_name))
                plt.savefig('./{}/{}/{}/{}_vs_SMNR_{}.pdf'.format(figs_dir, dirname, evaluation_mode, list_of_metrics[idx_display_metric].upper(), ssm_name))
            else:
                plt.figure()
                for j, model_name in enumerate(list_of_models_comparison):
