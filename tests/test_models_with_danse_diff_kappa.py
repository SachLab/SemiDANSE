...
if __name__ == "__main__":
    ...
    device = 'cpu'
    smnr_dB_test_arr = [-10.0, 0.0, 10.0, 20.0, 30.0]

    for smnr_dB_test in smnr_dB_test_arr:
        ...
        if mode == 'low' or mode == 'high':
            ...
            bias = None
            p = None
            evaluation_mode = 'full_opt_diff_kappa_smnr_{}dB_Ntrain_{}_Ttrain_{}_refactored'.format(smnr_dB_test, N_train, T_train)
        ...
        figs_dir = "figs_new_{}_diff_kappa".format(str(datetime.date.today()))
        os.makedirs('./{}/{}/{}'.format(figs_dir, dirname, evaluation_mode), exist_ok=True)

        kappa_arr = np.array([0.002, 0.005, 0.01, 0.02, 0.05])
        kappa_dict_arr = ["{}".format(kappa) for kappa in kappa_arr]
        ...
        metrics_multidim_mat = np.zeros((len(list_of_metrics), len(list_of_models_comparison), len(kappa_arr)))

        model_file_saved_dict = {#"danse":dict.fromkeys(kappa_dict_arr, {}), 
                                #"kalmannet":dict.fromkeys(kappa_dict_arr, {}),
                                #"dmm_st-l":dict.fromkeys(kappa_dict_arr, {}),
                                #"danse_supervised":dict.fromkeys(kappa_dict_arr, {}), 
                                "danse_semisupervised":dict.fromkeys(kappa_dict_arr, {}), 
                                }
        
        test_data_file_dict = {}

        for j, kappa_label in enumerate(kappa_dict_arr):
            test_data_file_dict[kappa_label] = "./data/synthetic_data/test_trajectories_m_{}_n_{}_{}_data_T_{}_N_{}_sigmae2_{}dB_smnr_{}dB.pkl".format(
                m, n, ssmtype, T_test, N_test, sigma_e2_dB_test, smnr_dB_test
                )
        ...
        for key in model_file_saved_dict.keys():
            for j, kappa_label in enumerate(kappa_dict_arr):
                ...
                nsup_j = int(kappa_arr[j] * N_train)
                N_train_actual = nsup_j if not nsup_j is None and saved_model in ["danse_supervised"] else N_train 

                if saved_model == "danse_semisupervised" and not nsup_j is None:
                    model_file_saved_dict[key][kappa_label] = glob.glob("./models/*{}_{}_*nsup_{}_m_{}_n_{}_*T_{}_N_{}*sigmae2_{}dB_smnr_{}dB*/*best*".format(
                        ssmtype, saved_model, nsup_j, m, n, T_train, N_train_actual, sigma_e2_dB_test, smnr_dB_test))[-1]
                elif saved_model == "KNetUoffline":
                    model_file_saved_dict[key][kappa_label] = glob.glob("./models/*{}_{}_*m_{}_*n_{}_*T_{}_N_{}*sigmae2_{}dB_smnr_{}dB*/*best*".format(
                        ssmtype, saved_model, m, n, T_train, N_train_actual, sigma_e2_dB_test, smnr_dB_test))[-1]
                else:
                    model_file_saved_dict[key][kappa_label] = glob.glob("./models/*{}_{}_*opt_gru_m_{}_*n_{}_*T_{}_N_{}*sigmae2_{}dB_smnr_{}dB*/*best*".format(
                        ssmtype, saved_model, m, n, T_train, N_train_actual, sigma_e2_dB_test, smnr_dB_test))[-1]

        test_logfile = "./{}/{}/{}/{}_test_{}_smnr_{}dB_T_{}_N_{}_diff_kappa.log".format(figs_dir, dirname, evaluation_mode, ssmtype, evaluation_mode, smnr_dB_test, T_test, N_test)
        test_jsonfile = "./{}/{}/{}/{}_test_{}_smnr_{}dB_T_{}_N_{}_diff_kappa.json".format(figs_dir, dirname, evaluation_mode, ssmtype, evaluation_mode, smnr_dB_test, T_test, N_test)

        print(model_file_saved_dict)

        for idx_kappa, kappa_label in enumerate(kappa_dict_arr):
            
            test_data_file_i = test_data_file_dict[kappa_label]
            learnable_model_files_i = {key: model_file_saved_dict[key][kappa_label] for key in model_file_saved_dict.keys()}
            ...
            metrics_dict_i = test_on_ssm_model(device=device, learnable_model_files = learnable_model_files_i, test_data_file=test_data_file_i, 
                ...
                dirname=dirname, kappa=kappa_arr[idx_kappa], figs_dir=figs_dir)
        ...
        for idx_display_metric in display_metrics:
            ...
            if list_of_metrics[idx_display_metric] != "time_elapsed":
                ...
                for j, model_name in enumerate(list_of_models_comparison):
                    plt.errorbar(kappa_arr, metrics_multidim_mat[idx_display_metric, j, :], fmt=list_of_display_fmts[j], yerr=metrics_multidim_mat[idx_display_metric+1, j, :],  linewidth=1.5, label="{}".format(model_name.upper()))
                plt.xlabel('$\\kappa$')
                ...
                plt.tight_layout()
                #plt.subplot(212)
                tikzplotlib.save('./{}/{}/{}/{}_vs_kappa_{}.tex'.format(figs_dir, dirname, evaluation_mode, list_of_metrics[idx_display_metric].upper(), ssm_name))
                plt.savefig('./{}/{}/{}/{}_vs_kappa_{}.pdf'.format(figs_dir, dirname, evaluation_mode, list_of_metrics[idx_display_metric].upper(), ssm_name))
            else:
                plt.figure()
                for j, model_name in enumerate(list_of_models_comparison):
                    plt.plot(kappa_arr, metrics_multidim_mat[idx_display_metric, j, :], list_of_display_fmts[j], linewidth=1.5, label="{}".format(model_name.upper()))
                plt.xlabel('$\\kappa$')
