...
def plot_state_trajectory_axes_all_L96(X,  
                                   X_est_DANSE=None, 
                                   X_est_DANSE_Supervised=None, 
                                   X_est_SemiDANSE=None,
                                   savefig=False, savefig_name=None):
    ...
    T_end = X.shape[0] #200
    K = 20
    idx_reqd = np.linspace(0, X.shape[1]-1, K).astype(np.int16)
    fig = plt.figure(figsize=(40,30))
    for i, id_ in enumerate(idx_reqd):
        #print(i, id_)
        plt.subplot(5, int(K // 5),i+1)
        plt.plot(X[:T_end, id_], '-',label='$\\mathbf{x}_{True} $')
        if X_est_DANSE is not None:
            plt.plot(X_est_DANSE[:T_end, id_], 'k--',label='$\\hat{\mathbf{x}}_{DANSE} $')
        if X_est_DANSE_Supervised is not None:
            plt.plot(X_est_DANSE_Supervised[:T_end, id_], 'm--',label='$\\hat{\mathbf{x}}_{DANSE-Sup.}  $')
        if X_est_SemiDANSE is not None:
            plt.plot(X_est_SemiDANSE[:T_end, id_], 'y--.',label='$\\hat{\mathbf{x}}_{SemiDANSE} $')
        plt.title("dim: {}".format(id_+1))
        plt.xlabel('$t$')
        plt.legend()
