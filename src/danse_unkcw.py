...
class DANSE_UNKCW(nn.Module):
    def __init__(
        ...
        super(DANSE_UNKCW, self).__init__()
        ...
        self.n_intermediate = (
            n_intermediate  # Number of intermediate optimization steps
        )
    ...
    def compute_meas_noise_cov(self, Yi_batch):
        # NOTE: Assuming measurement noise covariance matrix to be diagonal!!
        # NOTE: Moment matching-based approximate closed-form solution!!
        meas_noise_batch_hat = Yi_batch - torch.einsum(
            "ij,ntj->nti", self.H, self.mu_xt_yt_prev
        )
        sigmaw2_batch_hat = (
            torch.einsum(
                "...ii",
                torch.mean(
                    (
                        torch.einsum('nti,ntj->ntij', (meas_noise_batch_hat), (meas_noise_batch_hat))
        ...
        sigmaw2_batch_hat = torch.max(
            torch.empty_like(sigmaw2_batch_hat).fill_(1e-6), sigmaw2_batch_hat
        )
        assert (sigmaw2_batch_hat >= 0).all(), print(sigmaw2_batch_hat)
        Cw_batch_hat = torch.einsum('n,nij->nij',sigmaw2_batch_hat,
                                    torch.repeat_interleave(
            torch.eye(self.n_obs).unsqueeze(0),
            Yi_batch.shape[0],
            dim=0,
        ))
        #Cw_batch_hat = torch.repeat_interleave(
        #    (sigmaw2_batch_hat * torch.eye(self.n_obs)).unsqueeze(0),
        #    Yi_batch.shape[0],
        #    dim=0,
        #)
        return Cw_batch_hat

    def compute_marginal_mean_vars(self, mu_xt_yt_prev, L_xt_yt_prev, Cw_batch=None):
        Cw_batch_seq = (
            torch.repeat_interleave(Cw_batch.unsqueeze(1), L_xt_yt_prev.shape[1], dim=1)
            if Cw_batch is not None
            else torch.repeat_interleave(
                torch.repeat_interleave(
                    self.C_w.unsqueeze(0), L_xt_yt_prev.shape[1], dim=0
                ).unsqueeze(0),
                L_xt_yt_prev.shape[0],
                dim=0,
            )
        )

        #print(Cw_batch_seq.shape, self.H.device, self.mu_xt_yt_prev.device, self.mu_w.device)
    ...
    def compute_posterior_mean_vars(self, Yi_batch, Cw_batch):
        ...
        # self.L_xt_yt_current = self.L_xt_yt_prev - (torch.einsum('ntij,ntjk->ntik',
        #                    self.K_t, L_yt_current) @ torch.transpose(self.K_t, 2, 3))
        self.L_xt_yt_current = (
            (torch.eye(self.n_states) - self.K_t @ self.H)
            @ self.L_xt_yt_prev
            @ torch.transpose(torch.eye(self.n_states) - self.K_t @ self.H, 2, 3)
            + (
                torch.einsum("ntij,ntjk->ntik", self.K_t, Cw_batch_seq)
                @ torch.transpose(self.K_t, 2, 3)
            )
        )  # Joseph stabilizd version of the covariance measurement update from Simon, 2006
    ...
    def compute_predictions(self, Y_test_batch):
        ...
        Cw_test_batch_estimated = self.compute_meas_noise_cov(Yi_batch=Y_test_batch)
        mu_xt_yt_current_test, L_xt_yt_current_test = self.compute_posterior_mean_vars(
            Yi_batch=Y_test_batch, Cw_batch=Cw_test_batch_estimated
        )
        return (
            mu_xt_yt_prev_test,
            L_xt_yt_prev_test,
            ...
            Cw_test_batch_estimated,
        )

    def forward(self, Yi_batch, Cw_compute_start_flag=False):
        ...
        if Cw_compute_start_flag is True:
            self.compute_marginal_mean_vars(
                mu_xt_yt_prev=mu_xt_yt_prev, L_xt_yt_prev=L_xt_yt_prev, Cw_batch=None
            )
        else:
            with torch.no_grad():
                Cw_batch_estimated = self.compute_meas_noise_cov(Yi_batch=Yi_batch)
            
            self.compute_marginal_mean_vars(
                mu_xt_yt_prev=mu_xt_yt_prev,
                L_xt_yt_prev=L_xt_yt_prev,
                Cw_batch=Cw_batch_estimated,
            )
        logprob_batch = self.compute_logpdf_Gaussian(Y=Yi_batch) / (
            Yi_batch.shape[1] * Yi_batch.shape[2]
        )  # Per dim. and per sequence length
...
def train_danse(
    ...
    if save_chkpoints == "all" or save_chkpoints == "some":
        ...
        if logfile_path is None:
            training_logfile = "./log/danse_unkcw_{}.log".format(model.rnn_type)
        else:
        ...
        # Grid search
        if logfile_path is None:
            training_logfile = "./log/gs_training_danse_unkcw_{}.log".format(
                model.rnn_type
            )
        else:
            training_logfile = logfile_path
    ...
    try:
        for epoch in range(nepochs):
            ...
            val_mse_loss_Cw_epoch_sum = 0.0

            for i, data in enumerate(train_loader, 0):

                if epoch == 0:
                    Cw_compute_start_flag = True
                else:
                    Cw_compute_start_flag = False
    
                tr_Y_batch, tr_X_batch, _ = data
                optimizer.zero_grad()
                Y_train_batch = (
                    Variable(tr_Y_batch, requires_grad=False)
                    .type(torch.FloatTensor)
                    .to(device)
                )
                log_pY_train_batch = -model.forward(
                    Y_train_batch, Cw_compute_start_flag=Cw_compute_start_flag
                )
            ...
            with torch.no_grad():
                for i, data in enumerate(val_loader, 0):
                    ...
                    Y_val_batch = (
                        Variable(val_Y_batch, requires_grad=False)
                        .type(torch.FloatTensor)
                        .to(device)
                    )
                    Cw_val_batch = (
                        Variable(val_Cw_batch, requires_grad=False)
                        .type(torch.FloatTensor)
                        .to(device)
                    )
                    (
                        ...
                        Cw_val_batch_estimated,
                    ) = model.compute_predictions(Y_val_batch)
                    log_pY_val_batch = -model.forward(Y_val_batch, Cw_compute_start_flag=False)
                    val_loss_epoch_sum += log_pY_val_batch.item()
                    val_mse_loss_batch = mse_criterion(
                        val_X_batch[:, :, :].to(device), val_mu_X_filtered_batch
                    )
                    val_mse_loss_Cw_batch = mse_criterion(
                        Cw_val_batch, Cw_val_batch_estimated
                    )
                    # print statistics
                    val_mse_loss_epoch_sum += val_mse_loss_batch.item()
                    val_mse_loss_Cw_epoch_sum += val_mse_loss_Cw_batch.item()
            ...
            val_mse_loss_Cw = val_mse_loss_Cw_epoch_sum / len(val_loader)
            ...
            if tr_verbose is True and (((epoch + 1) % 50) == 0 or epoch == 0):
                print(
                    "Epoch: {}/{}, Training NLL:{:.9f}, Val. NLL:{:.9f}, Val. MSE:{:.9f}, Val. MSE Cw:{:.9f}, Time_Elapsed:{:.4f} secs".format(
                        epoch + 1,
            ...
            if (((epoch + 1) % 100) == 0 or epoch == 0) and save_chkpoints == "all":
                ...
                save_model(
                    ...
                    model_filepath
                    + "/"
                    + "danse_unk_cw_{}_ckpt_epoch_{}.pt".format(
        ...
        if save_chkpoints == "all" or save_chkpoints == "some":
            ...
            if best_model_wts is not None:
                model_filename = "danse_unk_cw_{}_ckpt_epoch_{}_best.pt".format(
        ...
        if save_chkpoints is not None:
            model_filename = "danse_unkcw_{}_ckpt_epoch_{}_latest.pt".format(
                model.rnn_type, epoch + 1
            )
