...
class DANSE_Regularized(nn.Module):
    ...
    def forward(self, Yi_batch_unsup, Yi_batch_sup, Xi_batch_sup):
        ...
        self.compute_marginal_mean_vars(mu_xt_yt_prev=mu_xt_yt_prev_sup, L_xt_yt_prev=L_xt_yt_prev_sup)
        mu_xt_yt_current_sup, L_xt_yt_current_sup = self.compute_posterior_mean_vars(Yi_batch=Yi_batch_sup)
        ...
        self.compute_marginal_mean_vars(mu_xt_yt_prev=mu_xt_yt_prev_unsup, L_xt_yt_prev=L_xt_yt_prev_unsup)
        ...
        log_pXTYT_batch_avg = logprob_batch_unsup.mean(0) + (lambda_ * logprob_batch_sup).mean(0)
        #log_pXTYT_batch_avg = (logprob_batch_unsup.sum(0) + logprob_batch_sup.sum(0)) / Yi_batch_unsup.shape[0]

        return log_pXTYT_batch_avg


def train_danse_regularized(model, options, train_loader, val_loader, nepochs, logfile_path, modelfile_path, save_chkpoints, device='cpu', tr_verbose=False):
    ...
    try:
        for epoch in range(nepochs):
            ...
            for i, data in enumerate(train_loader, 0):
                ...
                Y_train_batch_unsup, Y_train_batch_sup, X_train_batch_sup = split_joint_dataset(Y=Y_train_batch, X=X_train_batch, kappa=model.kappa)
                log_pXY_train_batch_avg = -model.forward(Y_train_batch, Y_train_batch_sup, X_train_batch_sup)
            ...
            with torch.no_grad():
                ...
                for i, data in enumerate(val_loader, 0):
                    ...
                    Y_val_batch_unsup, Y_val_batch_sup, X_val_batch_sup = split_joint_dataset(Y=Y_val_batch, X=X_val_batch, kappa=model.kappa)
                    ...
                    log_pXY_val_batch_avg = -model.forward(Y_val_batch, Y_val_batch_sup, X_val_batch_sup)
            ...
            if (((epoch + 1) % 100) == 0 or epoch == 0) and save_chkpoints == "all": 
                ...
                save_model(model, model_filepath + "/" + "danse_reg_{}_ckpt_epoch_{}.pt".format(model.rnn_type, epoch+1))
        ...
        if save_chkpoints == "all" or save_chkpoints == "some":
            ...
            if not best_model_wts is None:
                model_filename = "danse_reg_{}_ckpt_epoch_{}_best.pt".format(model.rnn_type, best_val_epoch)
                torch.save(best_model_wts, model_filepath + "/" + model_filename)
            else:
                model_filename = "danse_reg_{}_ckpt_epoch_{}_best.pt".format(model.rnn_type, epoch+1)
        ...
        if not save_chkpoints is None:
            model_filename = "danse_reg_{}_ckpt_epoch_{}_latest.pt".format(model.rnn_type, epoch+1)
...
def test_danse_regularized(test_loader, options, device, model_file=None, test_logfile_path = None):

    test_loss_epoch_sum = 0.0
    te_log_pXY_epoch_sum = 0.0 
    print("################ Evaluation Begins ################ \n")    
    
    # Set model in evaluation mode
    model = DANSE_Regularized(**options)
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(test_loader, 0):
            ...
            X_test_batch = Variable(te_X_batch, requires_grad=False).type(torch.FloatTensor).to(device)
            ...
            Y_test_batch_unsup, Y_test_batch_sup, X_test_batch_sup = split_joint_dataset(Y=Y_test_batch, X=X_test_batch, kappa=model.kappa)
            log_pXY_test_batch = -model.forward(Y_test_batch, Y_test_batch_sup, X_test_batch_sup)
            test_mse_loss_batch = criterion(te_X_batch, te_mu_X_filtered_batch)
            # print statistics
            test_loss_epoch_sum += test_mse_loss_batch.item()
            te_log_pXY_epoch_sum += log_pXY_test_batch.item()

        X_ref = te_X_batch[-1]
        X_hat_ref = te_mu_X_filtered_batch[-1]

    test_mse_loss = test_loss_epoch_sum / len(test_loader)
    test_NLL_loss = te_log_pXY_epoch_sum / len(test_loader)
